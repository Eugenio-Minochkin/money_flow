...
- Independent ordinary Telegram text expenses may finish parsing out of arrival order. At most two jobs for one user share the existing global worker limit; persistence and terminal delivery are serialized in readiness order. Stateful messages and callbacks are barriers. Durable capture ownership, paid request keys, Smart Save eligibility, currency ambiguity and closed-month validation remain authoritative.
