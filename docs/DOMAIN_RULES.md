...
- Independent ordinary Telegram text expenses may finish parsing out of arrival order. At most two jobs for one user share the existing global worker limit; persistence and terminal delivery are serialized in admission order, including controlled error/timeout finalization before the next expense can save. Stateful messages and callbacks are barriers. Durable capture ownership, paid request keys, Smart Save eligibility, currency ambiguity and closed-month validation remain authoritative.
...
- Telegram text and voice captures are durably identified by the owned `user_id + chat_id + message_id`; webhook retries reuse the original draft and cannot create a second expense. Accepted capture ownership is recorded before background processing; restart-resumable payloads remain until the ordered terminal delivery attempt finishes and are never emitted in diagnostics. A non-null payload is the unfinished-delivery marker even after draft creation or failure; recovery claims only the oldest unfinished capture per user. Delivery finalization clears it with the current claim version and live lease after a bounded terminal attempt. A final Telegram delivery failure releases the turn rather than blocking the queue forever; delivery success remains a separate diagnostic outcome. A crash after Telegram accepts a reply but before finalization can repeat the reply, never the expense.
- Telegram capture diagnostics use one internal `tgc_<base36>` key derived from the durable capture row and a one-based processing-attempt number. The key stays stable across webhook replay and restart recovery, while every reclaimed lease advances the attempt. Completion diagnostics also record only whether a terminal response was delivered; the key is operational-only and diagnostics must not expose raw user, chat, message, capture-row, text, transcript, or financial data.
- Money Flow handles financial messages, exports, admin commands, and callbacks only in a Telegram private chat. A group, supergroup, or channel receives only a neutral prompt to continue privately; no parser, export, financial mutation, or admin-statistics service may run for that update.
- Voice transcript repair is conservative: a recovered number/currency word boundary must be unambiguous, and compact numeric amounts are never reinterpreted as decimals. Deepgram smart formatting stays disabled so spoken number boundaries reach the parser without a formatting-induced decimal collapse. A processing loader terminalizes by edit, plain-edit fallback, or confirmed delete-before-send within one shared delivery budget. If that cascade fails, one bounded standalone attempt sends the same canonical result with a fresh signal, even if loader cleanup failed. Only confirmed delivery marks the response delivered; exhausted delivery raises a typed error for the queued-job alert path. A committed expense must never receive a generic processing-failed retry prompt, and delivery retries must never rerun parsing or saving. Diagnostics contain only stable stage/result enums, normalization/currency-recognition state, durable-capture state, and delivery mode, never raw transcripts or provider responses. Confirmed delivery stops further attempts; a lost Telegram acknowledgement cannot guarantee exactly-once message delivery.
- An explicit decimal amount keeps its value across text, voice, LLM fallback, and Telegram amount editing. Both `.` and `,` are decimal separators between digits; an exact supported ISO code or currency alias may accompany the edited amount, and a validated local decimal parse must not be replaced by an LLM result that collapses the separator.
...
- Image date/time components are interpreted in the user's current IANA timezone and stored as a UTC instant. A date without a time uses local noon so it remains on the stated calendar day. Invalid calendar values and nonexistent or ambiguous DST wall times stay in review without an invented timestamp. Initial and pre-save duplicate checks compare stored expenses and drafts in that same timezone.
- Photo-import summaries do not offer a financial confirmation for hidden candidates, and legacy summary-save callbacks open review instead of saving. `Add` / `Accounted` actions appear only beside the rendered fields of an owned unresolved draft; if that draft is missing or terminal, the review closes without financial actions. Duplicate review also states whether the match is possible or likely.
- Marking an image candidate as `already_accounted` atomically cancels its unresolved draft. Canonical draft confirmation must also reject legacy open drafts linked to an `already_accounted` candidate, so Inbox, recovery, stale callbacks, and concurrent confirmation cannot create a duplicate expense.
...
- Report delivery idempotency is tracked by user, report type, and period key. Sending may only happen after a delivery row is successfully claimed as `pending`. A fresh `pending` claim is never reclaimed concurrently; after 15 minutes an interrupted claim may be retried once, including outside the ordinary send window. Automatic delivery stops after two total attempts, and an exhausted stale claim becomes `failed` with `delivery_outcome_unknown`; only an explicit force send may reclaim it. Telegram cannot provide exactly-once delivery when a response is lost, so this bound limits a possible duplicate to one automatic retry.
