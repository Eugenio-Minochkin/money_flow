...
- Voice transcript repair is conservative: a recovered number/currency word boundary must be unambiguous, and compact numeric amounts are never reinterpreted as decimals. A processing loader terminalizes by edit, plain-edit fallback, or confirmed delete-before-send; if cleanup fails, diagnostics record only the mode and no duplicate terminal message is sent.
