...
- [x] PostgreSQL checks: concurrent ready drafts, month closure during FX, changed base currency and snapshot revalidation.
...
- [x] Final bounded Sol review; resolve concrete findings, open draft PR and verify exact-head unit/PostgreSQL CI. No merge or production operations.

Validation: local full suite 1753 tests / 1747 pass / 6 Windows POSIX skips. The 20-second lab delay completed B in 194.8 ms before A. Final Sol finding (queued callback ACK) is fixed with a real queue regression. Disposable PostgreSQL smoke and Linux unit CI passed on implementation commit 6e90e3d (run 35971377315). Draft PR #236 is open. Local Docker was unavailable; no persistent database or production operation was used.
