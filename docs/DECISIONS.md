...
Updated by #235 on 2026-09-25: `local_safe` keeps the local-primary route inside the configured `enabled` cohort. Category-only `local_reviewable` returns `local_review` immediately, with `other` and `needs_review=true`; unrelated review reasons still use the prior route. `shadow`, `off`, and rollout-excluded users keep LLM-primary behavior. `local_rejected` keeps LLM fallback or controlled failure.

Single-item category memory reads current expenses joined to explicitly human-confirmed single-item drafts of the same internal user. It requires three distinct draft confirmations, exact normalized description equality and unanimous current categories; even a non-eligible current row with a different category vetoes the hint. Existing saved-expense edits cannot reliably prove human provenance, so changed rows do not add positive evidence. Parser/Smart Save results cannot reinforce themselves. `other` never becomes an auto-save hint.

The lookup has a 100ms total connection/query deadline, reads at most 101 matching rows and abstains above 100. A user plus normalized-description digest index bounds the lookup without storing a second phrase map; complete normalized equality prevents digest collisions from matching. SQL lowercase/whitespace normalization is shared by query and index, with no fuzzy or punctuation folding. No positive cache is kept: edits/deletes are visible on the next lookup across workers. Timeout destroys an acquired client; a late connection is released. The caller retains fast review on failure. Index migration 026 is additive; its transactional build can briefly block expense writes at deployment and needs the ordinary separately authorized deployment window.
...
## 2026-09-23 - Router Aliases And Remaining Telegram Queue Latency

Issue #231 adds only exact `роутер` / `router` category aliases to `gear`. The local parser can resolve those purchases as `local_safe`, so the existing enabled cohort uses `local_primary`. `local_reviewable`, `shadow`, `off`, rollout-excluded users and currency ambiguity keep their existing routes; production rollout is unchanged.

At the time of #234, a deterministic characterization proved that a truly local-rejected A delayed a same-user local-safe B. Issue #233 changes that scheduling boundary as described below, without broadening the dictionary or changing parser rollout.

Production incident telemetry and the historical aggregate dictionary audit were not accessed during this implementation. No broader alias additions, schema changes, concurrency changes or production configuration changes are justified by the synthetic evidence here.

## 2026-09-24 - Independent Telegram Parse With Serialized Mutation

Issue #233 keeps each job counted against the existing global concurrency cap until all work settles. Up to two independent ordinary text jobs for one user can parse concurrently. Ready work acquires a per-user mutation lease before draft persistence and retains it through canonical save and terminal delivery. This permits local-safe B to save and reply before a slow LLM A, without committing in original admission order.

Stateful input uses FIFO barriers: commands, callbacks, onboarding, editor input, feedback, deletion, evidence, top-ups, planned and high-risk text cannot take the independent route. A pending barrier blocks later independent work. Parser mode, rollout and ambiguity handling still belong to the existing parser. No new environment setting, schema or paid-provider policy is introduced.

Capture reservation/claim/renewal, ownership version and stable paid request keys remain unchanged. Parent cancellation reaches the provider HTTP request; aborted mutation waiters cannot commit, and running jobs retain capacity until actual settlement. The canonical repository revalidates financial state under its existing locks. Each terminal result uses its own loader/message reference and the bounded fallback from #232; delivery failure never reparses or resaves.

Validation combines deterministic queue and bot tests with disposable PostgreSQL checks for concurrent saves, reserve-month closure and changed conversion inputs. The lab benchmark uses synthetic input and a 20-second delayed provider with fast mocked DB/Telegram; its sub-second B check is not a production SLA.
