...
The remaining mixed case is deliberately not claimed fixed: one truly `local_rejected` message waiting for LLM still blocks a later same-user `local_safe` message. `apps/api/test/telegramParserQueue.test.js` holds the real parser's injected LLM request behind a deferred promise and proves that another user can finish while the second same-user job cannot start until the first settles. This deterministic test characterizes the scheduling limitation without a real LLM call or a twenty-second sleep.

The queue currently owns the entire `processQueuedMessage` lifecycle. Moving parsing or adding a lane crosses admission reservations and capture lease ownership (`telegram.js`), stateful feedback/onboarding/topup/planned routes, stable provider request keys and capture completion (`expenseDraftService.js`), and cancellation: the parser currently uses its own LLM deadline rather than forwarding the job signal. `saveDraftAsExpense()` locks/revalidates the draft and financial months, returns existing expenses on replay, then reads the dashboard after commit; those guarantees do not alone define the validity of out-of-order user replies. No strict FIFO requirement for all independent financial saves has been established.

[Follow-up #233](https://github.com/Eugenio-Minochkin/money_flow/issues/233) specifies the separate change: preserve stateful barriers, bounded admission/parse resources, durable claims and paid idempotency; permit ready independent local-safe work to progress while another parse waits; revalidate through the canonical serialized mutation boundary and preserve terminal-message correlation. Its decisive test requires local-safe B to be saved and receive its terminal result **before** releasing LLM-required A. A synthetic twenty-second LLM delay must not leak into B's sub-second mocked-dependency benchmark, and replay/concurrent PostgreSQL reserve, month-close, FX and snapshot tests must pass. FIFO commit by original admission order alone would retain the delay and does not satisfy that criterion.
