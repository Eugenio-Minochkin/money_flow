...
- History search has a visible submit action and supports the keyboard search action. Show localized loading and retryable error feedback beside the results; keep previous rows, totals, and analytics hidden until the latest queued request finishes. History read errors must not replace Dashboard content.
