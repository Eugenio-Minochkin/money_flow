...
Run `node --test apps/api/test/telegramJobQueue.test.js apps/api/test/telegramParserQueue.test.js apps/api/test/telegramIndependentCapture.test.js apps/api/test/telegramStatefulQueue.test.js apps/api/test/telegram.test.js apps/api/test/expenseDraftService.test.js apps/api/test/expenseParser.test.js`. A genuinely local-rejected A holds an injected LLM request while local-safe B must save and deliver before A is released. Cover barriers, capture replay, cancellation settlement, pending mutation cancellation, capacity reservations and unchanged parser policies.
