...
Run focused capture checks with `npm.cmd test -- packages/shared/test/parser.test.js apps/api/test/voiceMoneyNormalization.test.js apps/api/test/voiceTranscriber.test.js apps/api/test/telegram.test.js apps/api/test/repository.test.js apps/api/test/db.test.js`. Cover joined STT number/currency boundaries, spoken decimal wording, true whole-number preservation, Deepgram requests without smart formatting, privacy-minimal voice diagnostics, and loader edit/plain-edit/delete fallback without duplicate terminal messages. Terminal-delivery regressions must cover explicit failure and missing acknowledgements, fresh bounded standalone delivery after the shared edit/delete deadline, queue-level alerts and fresh failure-notification signals, no generic retry prompt after a committed save, no recovery/replay recreation of top-up or planned drafts after delivery exhaustion while completed regular captures remain intact, no duplicate save on replay, and continuation of the next same-user job. PostgreSQL coverage uses only the disposable integration database via `npm.cmd run test:integration:postgres`.

Capture-correlation coverage must prove that the durable capture row keeps one opaque key across initial processing, replay, and restart recovery; a stale-lease reclaim advances the attempt exactly once even under concurrency; large PostgreSQL IDs are formatted without JavaScript number precision loss; and terminal completion records `delivered` or `not_delivered` only after the delivery cascade settles. Logs and analytics must not include raw internal or Telegram IDs, input text, transcripts, or financial data.
...
- Router alias coverage must prove exact RU/EN tokens resolve to `gear`, reject partial-word matches, preserve category conflict precedence and decimal amounts, and run the three-expense Telegram burst without an OpenAI call inside the enabled cohort. `apps/api/test/telegramParserQueue.test.js` verifies mixed LLM-first/local-safe-second independent parsing; bot-level tests require B to finish parsing while A is pending, but save and terminal delivery follow A then B. Cover three captures, parser error, timeout, delayed admission, replay, failed delivery finalization, and restart recovery with an earlier unexpired lease. PostgreSQL smoke verifies retained completed/failed captures, head-only recovery, and stale-claim rejection.
...
- Parser routing tests must prove that `local_safe` stays local primary and category-only `local_reviewable` returns immediate review without OpenAI or paid usage inside the existing enabled rollout. Other review reasons and off/shadow/excluded modes retain their routes. Personal hints use internal `usageUserId`, three distinct explicitly human-confirmed drafts, exact descriptions and no conflicting current category. Cover isolation, edits/deletion, parser-provenance exclusion, 101-row abstention, bounded connection/query failure and unchanged financial fields. `categoryMemory.test.js` tests the lookup lifecycle; PostgreSQL smoke verifies real SQL/index and synthetic overhead. Telegram keeps its Telegram ID as the rollout identity, but paid-provider reservations must use internal `users.id`. High-risk intents keep LLM fallback or controlled reject; parser-provided `other` stays review-only. `J3 30 GEL` must retain `j3` as description and 30 as amount; embedded identifier digits must not become money or hide a second amount.
- Telegram privacy-boundary tests must cover `group` and `supergroup` messages plus callbacks. They must prove that expense parsing, CSV export, and admin-statistics lookup do not run outside a private chat, while the existing private-chat export and admin flows remain available.
...
## Independent Telegram parsing (#233)

Run `node --test apps/api/test/telegramJobQueue.test.js apps/api/test/telegramParserQueue.test.js apps/api/test/telegramIndependentCapture.test.js apps/api/test/telegramStatefulQueue.test.js apps/api/test/telegram.test.js apps/api/test/expenseDraftService.test.js apps/api/test/expenseParser.test.js`. A slow A holds an injected parse delay while local-safe B becomes parsed-ready but cannot persist or deliver a terminal result. Releasing A must produce save A, terminal A, save B, terminal B. Cover barriers, capture replay/restart, cancellation settlement, active timeout excluding ordered wait, capacity reservations and unchanged parser policies.

Run `node apps/api/scripts/benchmark-telegram-queue.js` for the 20-second synthetic provider-delay experiment. Report parse wait, mutation wait, delivery and end-to-end durations separately, without input text, user identifiers or money values. This is a lab check with mocked fast persistence/Telegram, not a latency promise.

The disposable PostgreSQL smoke suite also covers concurrent ready draft saves, reserve/snapshot integrity and closed-month or conversion-input changes during delayed exchange-rate preparation. Never run it against a persistent database.

Timing compatibility: `processingTotalMs` / `activeProcessingMs` remain worker time; `endToEndTotalMs` remains queue wait plus worker time. `parseWaitMs` is the queue wait before starting the worker, `mutationWaitMs` measures the serialization wait, and `telegramResponseMs` covers Telegram requests. New `admissionMs` exposes pre-queue checks (including local eligibility evaluation); `captureEndToEndMs` includes those checks and reports the whole update duration. The benchmark measures dispatch-to-terminal time separately.
