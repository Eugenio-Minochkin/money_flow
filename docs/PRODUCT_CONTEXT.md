...
- Independent text expenses can parse concurrently, but saves and terminal replies follow admission order. A later ready result waits until the earlier result or controlled error has been finalized. Each result stays linked to its own input; commands, editing and other stateful actions preserve their order.
